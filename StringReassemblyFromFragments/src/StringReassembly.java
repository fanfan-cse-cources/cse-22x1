...
public final class StringReassembly {
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        while (!input.atEOS()) {
            addToSetAvoidingSubstrings(result, input.nextLine().toString());
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        final int location = text.indexOf('~');

        if (location != -1) {
            // If there is a <char>
            // Print String before <char>
            out.println(text.substring(0, location));
            // Recursive call String after <char>
            printWithLineSeparators(text.substring(location + 1, text.length()),
                    out);
        } else {
            out.println(text);
        }

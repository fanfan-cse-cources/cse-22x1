...
public final class Tokenizer {
    ...
    private static String nextWordOrSeparator(String text, int position) {
        ...
        boolean contains = separatorSet.contains(text.charAt(position));
        while (counter < text.length()
                && (contains == separatorSet.contains(text.charAt(counter)))) {
            counter++;
        }
    ...
    public static Queue<String> tokens(SimpleReader in) {
        ...
        while (!in.atEOS()) {
            ...
            while (position < text.length()) {
                String s = nextWordOrSeparator(text, position);

                if (!SEPARATORS.contains(s.substring(0, 1))) {
                    tokens.enqueue(s);
                }

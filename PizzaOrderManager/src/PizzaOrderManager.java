...
public final class PizzaOrderManager {
    ...
    private static void getPriceMap(String fileName,
        ...
        while (!in.atEOS()) {
            ...
            priceMap.add(str.substring(0, separate),
                    Integer.parseInt(str.substring(separate + 1)));
        }
    ...
    private static void putPrice(SimpleWriter output, int price) {
        ...
        final int centsLeft = price % centsToDollors;
        final int tenth = 10;

        if (centsLeft > tenth) {
            output.print(price / centsToDollors + "." + centsLeft);
        } else {
            output.print(price / centsToDollors + ".0" + centsLeft);
        }

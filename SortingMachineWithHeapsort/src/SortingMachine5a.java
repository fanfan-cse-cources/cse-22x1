...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        final int left = 2 * top + 1;
        final int right = 2 * top + 2;
        int root = top;

        /*
         * When left/right(or order) node smaller than last, set root to
         * left/right
         */
        if (left <= last
                && order.compare(array.entry(left), array.entry(top)) < 0) {
            root = left;
        }
        if (right <= last
                && order.compare(array.entry(right), array.entry(root)) < 0) {
            root = right;
        }
        /*
         * When root != top, exchange top and root, and make a recursive call
         */
        if (root != top) {
            array.exchangeEntries(top, root);
            siftDown(array, root, last, order);
        }
    }
...
//        this.heap = new Array1L<T>(this.heapSize);
    ...
    public final T removeFirst() {
        ...
        T x = this.heap.entry(0);
        this.heapSize--;
//        Array<T> a = new Array1L<>(this.heapSize);
//
//        for (int i = 0; i < this.heapSize; i++) {
//            a.setEntry(i, this.heap.entry(i + 1));
//        }
//
//        this.heap.transferFrom(a);

...
public final class CryptoUtilities {
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        if (n.compareTo(one) != 0) {
            remainder.copyFrom(n.divide(two));
            if (remainder.isZero()) {
                result = true;
            }
            n.multiply(two);
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        if (p.isZero()) {
            ...
            // remainder = p % 2
            remainder.copyFrom(p.divide(two));
            powerMod(n, p, m);
            ...
            NaturalNumber finalAnswer = new NaturalNumber2(n.divide(m));
            n.copyFrom(finalAnswer);
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        for (int i = 0; i < numberOfCandidates; i++) {
            if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
                ...
                /*
                 * w (witness) < n - 1 and [2, n – 1]
                 */
                NaturalNumber randRestriction = new NaturalNumber2(n);
                randRestriction.subtract(two);

                NaturalNumber w = new NaturalNumber2(
                        randomNumber(randRestriction));

                // When w <= 2, make w = 3
                if (w.compareTo(two) <= 0) {
                    w.copyFrom(three);
                }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        if (!isPrime2(n)) {
            /*
             * when n fails isPrime2 check, make a increment and recursive call
             */
